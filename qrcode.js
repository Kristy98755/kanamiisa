...
var qrcode = function() {
  var qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    _this.createTableTag = function(cellSize, margin) {
      ...
      var qrHtml = '';
      qrHtml += '<table style=" border-width: 0px; border-style: none; border-collapse: collapse; padding: 0px; margin: ' + margin + 'px;">';
      qrHtml += '<tbody>';
      for (var r = 0; r < _this.getModuleCount(); r += 1) {
        qrHtml += '<tr>';
        for (var c = 0; c < _this.getModuleCount(); c += 1) {
          qrHtml += '<td style=" border-width: 0px; border-style: none; border-collapse: collapse; padding: 0px; margin: 0px; width: ' + cellSize + 'px; height: ' + cellSize + 'px; background-color: ';
  ...
  var qrNumber = function(data) {
    ...
    _this.write = function(buffer) {
      var s = _data; var i = 0;
      while (i + 2 < s.length) { buffer.put(strToNum(s.substring(i, i + 3)), 10); i += 3; }
      if (i < s.length) {
        if (s.length - i == 1) { buffer.put(strToNum(s.substring(i, i + 1)), 4); }
        else if (s.length - i == 2) { buffer.put(strToNum(s.substring(i, i + 2)), 7); }
      }
    };
  ...
  var createDataURL = function(width, height, getPixel) {
    var gif = gifImage(width, height, getPixel);
    return 'data:image/gif;base64,' + btoaOutputStream(gif.toByteArray());
  };
  var gifImage = function(width, height, getPixel) {
    var _this = {};
    _this.width = width; _this.height = height;
    _this.getPixel = function(x, y) { return getPixel(x, y); };
    _this.toByteArray = function() {
      var buf = [];
      buf.writeByte(0x47); buf.writeByte(0x49); buf.writeByte(0x46); buf.writeByte(0x38); buf.writeByte(0x39); buf.writeByte(0x61);
      buf.writeShort(width); buf.writeShort(height);
      buf.writeByte(0xf7); buf.writeByte(0); buf.writeByte(0);
      for (var i = 0; i < 64; i += 1) { buf.writeByte(0xff); buf.writeByte(0xff); buf.writeByte(0xff); }
      buf.writeByte(0x2c); buf.writeShort(0); buf.writeShort(0); buf.writeShort(width); buf.writeShort(height);
      buf.writeByte(0);
      var minCodeSize = 8;
      buf.writeByte(minCodeSize);
      var enc = lzwEncode(minCodeSize, function(x, y) { return _this.getPixel(x, y); }, width, height);
      var i = 0;
      while (i < enc.length) { buf.writeByte(Math.min(255, enc.length - i)); buf.writeBytes(enc, i, Math.min(255, enc.length - i)); i += 255; }
      buf.writeByte(0x3b);
      return buf.toByteArray();
    };
    return _this;
  };
  var lzwEncode = function(minCodeSize, getPixel, width, height) {
    var clearCode = 1 << minCodeSize; var eofCode = clearCode + 1;
    var codeSize = minCodeSize + 1; var codePosition = 1 << codeSize; var codeMask = codePosition - 1;
    var entries = []; var bits = []; var buf = [];
    var addCode = function(code) {
      for (var i = 0; i < bits.length; i += 1) { buf.push(bits[i]); }
      bits = []; bits.push(code); codeSize++; codePosition <<= 1; codeMask = codePosition - 1;
    };
    var clear = function() { bits = []; codeSize = minCodeSize + 1; codePosition = 1 << codeSize; codeMask = codePosition - 1; entries = []; for (var i = 0; i < clearCode; i += 1) { entries.push([i]); } };
    clear();
    addCode(clearCode);
    var x = 0, y = 0;
    var last = getPixel(x, y);
    var current;
    while (true) {
      x += 1; if (x >= width) { x = 0; y += 1; if (y >= height) { addCode(eofCode); break; } }
      current = getPixel(x, y);
      var code = -1; var i;
      for (i = 0; i < entries.length; i += 1) {
        if (entries[i][0] == last && entries[i].length == 2) { code = entries[i][1]; break; }
        else if (entries[i][0] == last && entries[i].length == 1) { code = i; break; }
      }
      if (code >= 0) {
        last = current;
        if (code < codePosition) { addCode(code); } else { addCode(codePosition); addCode(current); }
        if (entries.length < 4096) {
          if (entries.length == codePosition) { codePosition <<= 1; codeMask = codePosition - 1; codeSize += 1; }
          entries.push([last, current]);
        }
      } else {
        if (last >= 0) { addCode(last); }
        if (entries.length < 4096) { if (entries.length == codePosition) { codePosition <<= 1; codeMask = codePosition - 1; codeSize += 1; } entries.push([last, current]); }
        last = current;
      }
    }
    return buf;
  };
  var btoaOutputStream = function(stream) {
    var data = stream.toByteArray(); var base64 = '';
    var buffer = 0; var buflen = 0;
    for (var i = 0; i < data.length; i += 1) {
      buffer = (buffer << 8) | (data[i] & 0xff);
      buflen += 8;
      while (buflen >= 6) { base64 += encode(buffer >>> (buflen - 6)); buflen -= 6; }
    }
    if (buflen > 0) { base64 += encode(buffer << (6 - buflen)); }
    while (base64.length % 4 != 0) { base64 += '='; }
    return base64;
  };
  var encode = function(n) {
    if (n < 26) { return String.fromCharCode(0x41 + n); }
    else if (n < 52) { return String.fromCharCode(0x61 + (n - 26)); }
    else if (n < 62) { return String.fromCharCode(0x30 + (n - 52)); }
    else if (n == 62) { return '+'; }
    else if (n == 63) { return '/'; }
    else { throw 'n:' + n; }
  };
